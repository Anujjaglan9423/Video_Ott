...
const NewLogin = () => {
  ...
  const onSuccess = (data, values) => {
    if (data.data.status == "OK") {
      ...
      setAlert({
        msg: data.data.msg || "Your mobile number is not registered",
        show: true,
        color: "red",
      });
    }
  ...
  const onVerifyOtpError = (data, values) => {
    console.log(data.data);
  ...
  const loginUserApi = async () => {
    ...
    return await axios.post(Config.LoginApi, body, {
      headers: {
  ...
  const { mutate: loginUser, isLoading } = useMutation(loginUserApi, {
    onSuccess,
    onError,
  });
        ...
        <Wrapper>
          ...
          <Title>
            {isOtpVisible ? (
              ...
              <>
                <h1 className="text-2xl">{t("WelcomeAgain")}</h1>
...
const LogoName = tw.div`font_f text-secondaryfont-blue md:text-2xl text-lg  mb-2  md:mb-4  leading-8   text-center`;
...
const Line = tw.div`w-20 h-0.5 mx-auto bg-secondaryfont-blue`;
const Term = tw.h3`w-full text-gray-400 mt-4  text-base justify-center font-normal flex flex-row`;
const Resend = tw.h3`w-full text-gray-400 mt-8  text-base text-center font-normal`;
